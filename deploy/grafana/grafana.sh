...
# 询问并设置账号和密码
read -rp "[Grafana] Please input the username (default: admin): " username
username=${username:-admin}
read -rp "[Grafana] Please input the password (default: perf@JMeter#1024): " password
password=${password:-perf@JMeter#1024}

# 询问镜像仓库
read -rp "[Grafana] Please input the image repository (default: docker.io): " repository
...
configure_datasource_influxdb2() {
  read -rp "[InfluxDB2] Please input the hostname [default: influxdb]: " hostname
  hostname=${hostname:-influxdb}
  read -rp "[InfluxDB2] Please input the port [default: 8086]: " port
  port=${port:-8086}
  ...
  read -rp "[InfluxDB2] Please input the token: " token
...
# 询问是否配置数据源
read -rp "Do you want to configure datasource InfluxDB2 for Grafana? [Y/N]: " answer
if [[ "${answer}" =~ ^[Yy] ]]; then
  configure_datasource_influxdb2
else
